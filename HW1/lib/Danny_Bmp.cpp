...
void readBmpImageHeader(char* fileName, bmpheader *bmpStruct, bool showInfo){
  ...
  uint32_t size = getInfo(info, 35, 38);
  ...
  bmpStruct->Width = getInfo(info, 19, 22);
  bmpStruct->Height = getInfo(info, 23, 26);
  ...
  fread(plalette, sizeof(uint8_t), usedColorSize*4, f_r);

  uint8_t image_data[size];
...
void readBmpImageData(char* fileName, bmpheader *bmpStruct, uint8_t* img_data){
  ...
  FILE* f_r = fopen(fileName, "rb");
  fread(info, sizeof(uint8_t), BMP_HEADER_SIZE, f_r);
  bmpStruct->Plalette = (uint8_t*) malloc(bmpStruct->UsedColorsSize*4);
  fread(bmpStruct->Plalette, sizeof(uint8_t), bmpStruct->UsedColorsSize*4, f_r);

  // Read Data
  fread(img_data, sizeof(uint8_t), bmpStruct->BitmapDataSize, f_r);
  fclose(f_r);
}
...
void writeBmpImage(char* fileName, bmpheader *bmpStruct, uint8_t* img_data){
  ...
  fwrite(bmpStruct->Plalette, sizeof(uint8_t), bmpStruct->UsedColorsSize*4, f_w);
  fwrite(img_data, sizeof(uint8_t), bmpStruct->BitmapDataSize, f_w);
  fclose(f_w);
}
